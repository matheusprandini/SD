...
#Arestas têm nomes de dois vértices(dois inteiros positivos), um peso(real), uma flag indicando se é direcionado(do primeiro para o segundo) ou se é bi-direcional, e uma descrição(string).
struct Aresta {
...
#Exceção VerticeNaoExistente é disparada ao tentar remover um vértice não existente no grafo. Também é disparada ao tentar criar uma aresta que toca um vértice não existente, além da tentativa de ou modificar, ou ler, ou exibir informações de um vértice que não existe no grafo.
exception VerticeNaoExistente 
{
}

#Exceção ArestaExistente é disparada ao tentar criar uma aresta ligando dois vértices que já possuem uma aresta de ligação.
exception ArestaExistente
{
}

#Exceção ArestaNaoExistente é disparada ao tentar remover uma aresta ligando dois vértices que não possuem uma aresta de ligação. Também é disparada na tentativa de ou modificar, ou ler, ou exibir informações de uma aresta que não existe no grafo.
...
#criarAresta -> Uma aresta só pode ser adicionada se não existir outra tocando os mesmos dois vértices. Além disso, ambos os vértices tocados pela aresta devem existir no grafo.


##REMOÇÃO##

#removerVertice -> Um vértice só pode ser removido se existir no grafo. Além disso, é necessário remover todas as arestas que tocam o vértice removido.
...
#modificarVertice -> Modificação realizada a partir do nome/identificador do vértice. Além disso, o nome do vértice não pode ser alterado.

#modificaçãoAresta -> Modificação realizada a partir do nome/identificador da aresta. Além disso, uma aresta não pode ter seus vértices mudados.
...
service OperacoesGrafo {
    ...
    string criarVertice(1:i64 nome, 2:i64 cor, 3:string descricao, 4:double peso) throws 			(1:VerticeExistente VE),
    
    string criarAresta(1:i64 nome, 2:i64 vertice1, 3:i64 vertice2, 4:double peso, 5:bool 			direcionado, 6:string descricao) throws (1:ArestaExistente AE, 				2:VerticeNaoExistente VNE),
    ...
    ##MODIFICAÇÃO##
    string modificarVertice(1:i64 nome, 2:i64 cor, 3:string descricao, 4:double peso) throws 			(1:VerticeNaoExistente VNE),
